...
public class ClientConfig {
    ...
    @Bean
    public HttpInvokerProxyFactoryBean reviewInvoker(AuthenticationSimpleHttpInvokerRequestExecutor requestExecutor) {
        HttpInvokerProxyFactoryBean invoker = new HttpInvokerProxyFactoryBean();
        invoker.setHttpInvokerRequestExecutor(requestExecutor);
        invoker.setServiceUrl("http://localhost:8080/review");
        invoker.setServiceInterface(ReviewServiceInterface.class);
        return invoker;
    }
}
